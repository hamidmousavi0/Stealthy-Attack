...
# names of model in models folder
...
parser.add_argument('--data_path',
                    default='/media/hamid/ali/RESEARCH/DeepFool_weight_attack(ICCAD)/data/mnist',
                    type=str,
                    help='Path to dataset')
# name of dataset based on choose
parser.add_argument(
    '--dataset',
    type=str,
    default='mnist',
...
# name of architecture that be used
...
parser.add_argument('--save_path',
                    type=str,
                    default='./save/quan_small_cnn_mnist',
...
parser.add_argument('--robustness',
                    dest='robustness',
                    action='store_true',
                    help='evaluate robustness model on validation set')
#############################################################
parser.add_argument(
    '--fine_tune',
    dest='fine_tune',
...
# random seed
parser.add_argument('--manualSeed', type=int, default=7254, help='manual seed')
# quantization
parser.add_argument(
    '--quan_bitwidth',
    type=int,
...
parser.add_argument(
    ...
    type=str,
    default='clean',
    choices=['clean','noise','rotate','blur','erase'],
    help='Choose between corrupt model.')
##########################################################################
...
def main():
    ...
    print_log("cudnn  version : {}".format(torch.backends.cudnn.version()),
              log)
    print_log("status of input data : {}".format(args.corrupt),log)
    ...
    net = models.__dict__[args.arch](True)
    print_log("=> network :\n {}".format(net), log)
    ...
    if args.evaluate:
        ...
        return
    if  args.robustness:
        robustness = dpf.robustness(test_loader,net)
        print_log('robustness of net on test loader={}'.format(robustness),log)
    # Main loop
    ...
    log.close()
    torch.save(net,"/media/hamid/ali/RESEARCH/DeepFool_weight_attack(ICCAD)/lenet.pth")
...
def perform_attack(attacker, model, model_clean, train_loader, test_loader,
    ...
    for _, (data, target) in enumerate(test_loader):
        ...
        # data = data.squeeze().view(-1, 32 * 32) # just for FC
    ...
    # evalaute the robustness of clean model
    # robustness_original = dpf.robustness(test_loader,model)
    print_log("validation accuracy top1 on {} data before attack:{}".format(args.corrupt,val_acc_top1),log)
    print_log("validation accuracy top5 on {} data before attack:{}".format(args.corrupt,val_acc_top5),log)
    # print_log("robustness {} data before attack:{}".format(args.corrupt, robustness_original), log)
    ...
    # last_robustness = robustness_original
    
    for i_iter in range(N_iter):
        print_log('**********************************', log)
        if not random_attack:
            attack_log = attacker.progressive_bit_search(model, data, target,test_loader)
        else:
        ...
        try:
            ...
            # print_log('robustness before attack: {:.4f}'.format(attacker.robustness.item()),
            #           log)
            # print_log('robustness after attack: {:.4f}'.format(attacker.robustness_max), log)
        except:
            pass
        ...
        robustness_afterattack= dpf.robustness(test_loader, model)
        print_log("validation accuracy top1 on {} data after attack:{}".format(args.corrupt,val_acc_top1),log)
        print_log("validation accuracy top5 on {} data after attack:{}".format(args.corrupt,val_acc_top5),log)
        print_log("robustness {} data after attack:{}".format(args.corrupt, robustness_afterattack), log)
        tmp_df = pd.DataFrame(output_summary, columns=['top-1 output'])
        ...
        # rob_drop = last_robustness - robustness_afterattack
        last_val_acc_top1 = val_acc_top1
        # last_robustness = robustness_afterattack
